...
(function($) {
	...
	$('.dropdown-toggle').attr('href', 'javascript:;');
    ...
    $('.box_welcome').click(function(){
        window.location.assign('comience-aqui.php');
    });
    ...
    $('nav [href$=".php"]').each(function(index){
   ...
   $.Scrollax();
		...
		$(window).scroll(function(){
			...
			if (st > 150) {
				if ( !navbar.hasClass('scrolled') ) {
					...
					$('#logotipo').attr('src', 'images/logo-black-v2.png');
				}
			} 
			if (st < 150) {
				if ( navbar.hasClass('scrolled') ) {
					navbar.removeClass('scrolled sleep');
					$('#logotipo').attr('src', 'images/logo-v2.png');
				}
			} 
			if ( st > 350 ) {
	...
	for(i = 0; i < 6; i++) {
		...
		} while(image_.includes(rand) == true);
    ...
    $.ajax({
      ...
      method: 'GET'
        ...
        $('.ftco-gallery .image-popup').each(function(index){
            $(this).attr('href', 'images/gallery/' + data[index].image).css('background-image', 'url(images/gallery/' + data[index].image + ')').find('.icon-instagram').removeClass('icon-instagram').addClass('icon-search');
        });
